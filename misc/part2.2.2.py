...
def action(state):
    ...
    if(state.position==0):   #Pos::center #Availaible actions : Up, Down, Right, Left, None, shoot, stay, hit
        ...
        if(state.monsterState==1):  #Dormant state
            ...
            + 0.85 * 0.2 *(COST -COST+ REWARD[state2.show()] + GAMMA*utilities[state2.show()])
            + 0.15 * 0.8 *(COST -COST+ REWARD[state3.show()] + GAMMA*utilities[state3.show()])
            ...
            StayCost= (0.85 * 0.5 *(COST -COST+ REWARD[state1.show()] + GAMMA*utilities[state1.show()])
